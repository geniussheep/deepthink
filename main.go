...
import (
	...
	"github.com/winlion/deepthink/controller"
...
func registerRouter(router *gin.Engine) {
	...
	new(controller.ChannelController).Router(router)
...
func main() {
	...
	for k, ds := range cfg.Datasource {
		...
		e.SetMaxOpenConns(n)
		err = e.Sync2(new(entity.User), new(entity.Channel), new(entity.Article), new(entity.Config), new(entity.RefRoleRes), new(entity.Resource), new(entity.Role))
		if err != nil {
			fmt.Println("data source init error", err.Error())
			return
		}
